// dom15-calificaciones_mp.js
// Registro de Almas y Calificaciones en la Cripta
...
function agregarAlumno() {
   ...
   if (nombre === '') {
       divError.textContent   = '👻 Por favor ingresa el nombre del alma errante.';
   ...
   alumnos.forEach((alumno, index) => {
       ...
       fila.innerHTML = `
           ...
           <td class="${aprobado ? 'estado-aprobado' : 'estado-reprobado'}">
               ${aprobado ? 'Aprobado 🕯️' : 'Reprobado ⚰️'}
           </td>
           <td>
...
function calcularEstadisticas() {
   ...
   if (alumnos.length === 0) {
       divError.textContent   = '🕸️ Agrega al menos un alma antes de invocar las estadísticas.';
       divError.style.display = 'block';
       return;
   }
...
// Enter en los campos de ingreso agrega el alma
window.onload = () => {
